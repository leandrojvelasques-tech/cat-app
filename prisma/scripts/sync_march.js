...
async function main() {
  const marchMembers = [
    ...
    { num: 4001, last: "THOMAS", first: "KARINA ELIZABETH", amount: 6000, date: "2026-03-06" },
    { num: 4002, last: "URIBE", first: "JOSE RUPERTO", amount: 6000, date: "2026-03-03" },
    ...
    { num: 4003, last: "ZENI", first: "MARIELA", amount: 3000, date: "2026-03-01" },
  ]

  console.log("Cleaning up March 2026 payments...")
  ...
  for (const item of marchMembers) {
    let member = await prisma.member.findFirst({
      where: {
        OR: [
          ...
          { AND: [
            { lastName: { contains: item.last } },
            { firstName: { contains: item.first } }
          ]}
        ]
      }
    })
    ...
    if (!member) {
      console.log(`Creating: ${item.first} ${item.last} (#${item.num})`)
      member = await prisma.member.create({
        data: {
          memberNumber: item.num,
          firstName: item.first,
          lastName: item.last,
          dni: "TEMP-" + item.num + "-" + Date.now().toString().slice(-4),
          status: "ACTIVE",
          type: "ACTIVO",
    ...
    console.log(`Recording March for ${member.lastName}`)
    try {
      await prisma.membershipFee.create({
        data: {
          ...
          paymentStatus: "PAID",
          notes: "Planilla Marzo"
        }
      })
    } catch (e) {
      console.error(`Could not record fee for ${member.lastName}: ${e.message}`)
    }
  }
