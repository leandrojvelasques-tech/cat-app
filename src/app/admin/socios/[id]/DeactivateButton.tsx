    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-zinc-900 border border-white/10 rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <p className="text-zinc-400 text-sm mb-6">
          El socio será transferido al <strong className="text-white">Archivo de Socios</strong>. Podrás consultar su ficha allí, pero ya no aparecerá en el Directorio activo.
        </p>
        ...
        <div className="space-y-4 mb-8">
          <div className="space-y-2">
            ...
            <select 
              className="w-full bg-zinc-950 border border-white/10 rounded-xl px-4 py-3 text-zinc-200 focus:outline-none focus:border-red-500/50 appearance-none cursor-pointer"
              value={reason}
        ...
        <div className="flex gap-3">
          <button 
            onClick={() => setIsOpen(false)}
            className="flex-1 bg-white/5 hover:bg-white/10 py-3 rounded-xl font-medium transition-colors border border-white/5 text-zinc-300"
          >
            Cancelar
          </button>
          <button 
            ...
            disabled={isPending}
            className="flex-1 bg-red-600 hover:bg-red-500 disabled:opacity-50 py-3 rounded-xl font-medium transition-all shadow-lg shadow-red-900/20 flex items-center justify-center gap-2 text-white"
          >
